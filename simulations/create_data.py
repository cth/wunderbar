...
for i in range(0,number_of_individuals):
    ...
    individual = []
    snps = []
    for j in range(0,number_of_snps):       
        if(with_missing):
            snp = randrange(0,4,1)
            if(snp==0): #only 12.5 % chance for missingdata
                seed()
                snp = randrange(0,4,1)
            snps.append(snp%4)
        else:
        ...
        if(random()<mismatch_rate):
            while mismatchsnp == snps[b]:
                if(with_missing):
                    ...
                    if(mismatchsnp==0): #only 12.5 % chance for missingdata
                        seed()
                        mismatchsnp = randrange(0,4,1)%4
                else:
...
for ind in chip:
    ...
    formatted_ind.extend(map(recoder,ind[6:(6+number_of_snps)]))
